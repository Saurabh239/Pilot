...
shinyUI(
  fluidPage( downloadButton('downloadPlot', 'Download Plot'),
             theme = "bootstrap.css",
             ...
             sidebarLayout(
               sidebarPanel( 
                 ...
                 conditionalPanel(
                   condition = "input.PlotChoiceInput == 'CA'",
                   selectInput(
                     "ComparisonInput",
                     label = "Comparison Type",
                     choices = c( "Roll Rate" = "RR",
                                  "UPB Percentage" = "UP"                                  
                     ),
                     selected = NULL                   
                   )
                 ...
                 conditionalPanel(
                   condition = "input.ComparisonInput == 'RR'
                   || input.ComparisonInput == 'UP'",
                   selectInput(
                     "BucketInput",
                     label = "Buckets Combination",                    
                     choices = c( "LTV & FICO" = "LF",
                                  "ALS" = "ALS",
                                  "ALL" = "ALL"                               
                     ),
                     selected = NULL                   
                 ...
                 uiOutput("PNumControl"),
                 
                 uiOutput("OTypeControl"),
                 
                 uiOutput("PTypeControl"),
                 
                 conditionalPanel(
                   condition = "input.PlotChoiceInput == 'CA' 
                        && input.BucketInput == 'LF'",
                   selectInput(
                     "FicoBucketInput",
                     label = "FicoBucket",
                     choices = levels(FicoLtvDlqBkt1$FicoBucket)                    
                   )
                 ),
                 ...
                 conditionalPanel(
                   ...
                   selectInput(
                     "LtvBucketInput",
                     label = "LtvBucket",
                     choices = levels(FicoLtvDlqBkt1$LtvBucket)
                   )
                 ),
                 
                 conditionalPanel(
                   condition = "input.PlotChoiceInput == 'CA' 
                      && input.BucketInput == 'ALS'",
                   selectInput(
                     "LSBucketInput",
                     label = "LSBucket",
                     choices = (levels(AlsDlqBkt1$LSBucket))                   
                   )
                 ),  
                 
                 uiOutput("DlqStatusControl"),
                 ...
                 conditionalPanel(
                   ...
                   selectInput(
                     "DlqChoiceInput",
                     label = "Dlq Choice",
                     choices = c("AggDlqCurrBalPer","AggDlq30BalPer",
                                 "AggDlq60BalPer", "AggDlq90BalPer",
                                 "AggDlq120BalPer", "AggDlq150BalPer",
                                 "AggDlq180PlusBalPer","AggBalanceXPer"
                     )
                   )
                 ),                                              
                 
                 actionButton("ResetInput", label = "Reset")                               
...
From: Singh, Saurabh 
Sent: Thursday, October 16, 2014 8:00 PM
To: Kardam, Preksha; Bharadwaj, Vineet
Subject: server.r
